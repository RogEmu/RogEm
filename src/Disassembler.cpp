...
static std::string formatAssembly(uint32_t pc, Instruction i, const InstructionData &data)
{
    ...
    result = std::regex_replace(result, std::regex("%rs"), m_registerNames[i.r.rs]);
    result = std::regex_replace(result, std::regex("%rt"), m_registerNames[i.r.rt]);
    result = std::regex_replace(result, std::regex("%rd"), m_registerNames[i.r.rd]);
...
void Disassembler::debugState(uint32_t pc, const uint32_t *registers)
{
    ...
    for (int i = 0; i < 32; i++) {
        std::cout << fmt::format("    r{:02d}: 0x{:08x}", i, registers[i]);
        if (i % 4 == 3) std::cout << "\n";
        else std::cout << "    ";
    }
}
